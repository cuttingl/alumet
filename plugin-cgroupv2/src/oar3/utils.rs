...
mod tests {
    ...
    fn test_is_accessible_dir() {
        ...
        let root = tmp.join("test-alumet-plugin-oar/kubepods-list-metrics");
    ...
    fn test_gather_value_with_invalid_data() {
        let tmp = std::env::temp_dir();
        let root = tmp.join("test-alumet-plugin-oar/kubepods-invalid-gather.slice/");
    ...
    fn test_gather_value_with_valid_values() {
        ...
        let mut cgroup =
            CgroupV2MetricFile::new("testing_pod".to_string(), path_cpu, path_memory, file_cpu, file_memory);
    ...
    fn test_list_all_file() {
        ...
        let non_existent_path = root.join("non_existent");
        let result = list_all_file(&non_existent_path);
        assert!(result.is_ok());
