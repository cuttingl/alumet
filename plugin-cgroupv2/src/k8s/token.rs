...
mod tests {
    ...
    async fn test_refresh_with_valid_token() {
        ...
        let root = tmp.join("test-alumet-plugin-k8s/var_1/");
        ...
        token.path =
            Some("/tmp/test-alumet-plugin-k8s/var_1/run/secrets/kubernetes.io/serviceaccount/token_1".to_string());
        let result = token.refresh().await;

        assert!(result.is_ok());
        let refreshed_token = result.unwrap();
        assert_eq!(refreshed_token, content);
    ...
    async fn test_refresh_with_missing_exp() {
        let tmp = std::env::temp_dir();
        let root = tmp.join("test-alumet-plugin-k8s/var_2/");
        ...
        let mut token = Token::new(TokenRetrieval::File);
        token.path =
            Some("/tmp/test-alumet-plugin-k8s/var_2/run/secrets/kubernetes.io/serviceaccount/token_2".to_string());
        let result = token.refresh().await;
